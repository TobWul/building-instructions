...
The color used for backgrounds is `#F5F5F5` or `layer.1.default`. If we want to change the background color, we would have to change it in every place it is used.
